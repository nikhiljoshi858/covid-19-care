...
weightsPath = settings.BASE_DIR + '/models/yolov3.weights'
configPath = settings.BASE_DIR + '/models/yolov3.cfg'
...
#                   The bounding box will be red for violators and green for non-violators. Like the image_view or webcam_view,
#                   the video is not displayed directly, but it can be downloaded and played by the user locally
# Example Call:		Called by Django when the particular URL is called by the user
...
def webcam_view(request):
    if request.method == "POST":
        ...
        polygonOnex = int(float(request.POST['polygonOnex']))
        polygonOney = int(float(request.POST['polygonOney']))
        polygonTwox = int(float(request.POST['polygonTwox']))
        polygonTwoy = int(float(request.POST['polygonTwoy']))
        polygonThreex = int(float(request.POST['polygonThreex']))
        polygonThreey = int(float(request.POST['polygonThreey']))
        polygonFourx =int( float(request.POST['polygonFourx']))
        polygonFoury = int(float(request.POST['polygonFoury']))
        heightOnex = int(float(request.POST['heightOnex']))
        heightOney = int(float(request.POST['heightOney']))
        heightTwox = int(float(request.POST['heightTwox']))
        heightTwoy = int(float(request.POST['heightTwoy']))
        polygon = [[polygonOnex,polygonOney],[polygonTwox,polygonTwoy],[polygonThreex,polygonThreey],[polygonFourx,polygonFoury],[heightOnex,heightOney],[heightTwox,heightTwoy]]
        copied_image = image.copy()
        mouse_pts = polygon
        ...
        context['sd_view'] = uri1
        context['birds_eye_view'] = uri2
        return JsonResponse({'sd_view':uri1,'bdv':uri2})
